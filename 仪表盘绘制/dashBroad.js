const bgimage = require('@src/images/yibiaopan.png');
export default class DashBoard {
  ...
  drawScore(score) {
    ...
    this.ctx.fillText(score, this.rx - (score == 100 ? 90 : score < 10 ? 50 : 70), this.ry + 10);
    this.ctx.beginPath();
    this.ctx.font = 'bold 40px Microsoft Yahei';
    this.ctx.fillText('分', this.rx + (score == 100 ? 50 : score < 10 ? 10 : 30), this.ry + 5);
    this.ctx.beginPath();
    this.ctx.font = '30px Microsoft Yahei';
    this.ctx.fillText('- 信用评分 -', this.rx - 85, this.ry + 90);
  }
  // 画表盘
  async drawBg() {
  ...
  drawPoint(score) {
    ...
    this.ctx.arc( this.rx + this.radius * Math.cos( toRadian ), this.ry + this.radius * Math.sin( toRadian ), 8, 0, Math.PI * 2 );
    this.ctx.stroke();
    this.ctx.beginPath();
    this.ctx.strokeStyle = '#ffffff';
    this.ctx.lineWidth = 6;
    this.ctx.arc( this.rx + this.radius * Math.cos( toRadian ), this.ry + this.radius * Math.sin( toRadian ), 15, 0, Math.PI * 2 );
    this.ctx.stroke();
  }
  // 生成仪表盘
  init(score) {
    return new Promise((resolve, reject) => {
      ...
      image.src = bgimage;
      image.onload = () => {
        this.ctx.drawImage(image, 50, 50, 500, 323.2);
        this.drawArc(score);
